...
//	LinearIndex_m<L2<float> >* linear;
    flann::LinearIndex<L2<float> > *linear;
//	KDTreeIndex_m<L2<float> >* kdtree;
    flann::KDTreeIndex<L2<float> > *kdtree;
//	KMeansIndex_m<L2<float> >* kmeans;
    flann::KMeansIndex<L2<float>> *kmeans;
    RBCIndex *rbc;
...
//			linear = new LinearIndex_m<L2<float> >(mat);
                linear = new flann::LinearIndex<L2<float>>(mat);
                linear->buildIndex();
                break;
            case 2:
//                kdtree = new KDTreeIndex_m<L2<float> >(mat);
                kdtree = new flann::KDTreeIndex<L2<float>>(mat);
                kdtree->buildIndex();
                break;
            case 3:
//                kmeans = new KMeansIndex_m<L2<float> >(mat);
                kmeans = new flann::KMeansIndex<L2<float>>(mat);
                kmeans->buildIndex();
                break;
            case 4:
